...
5. 当执行到地址指令为`6`、`7`、`8`时，将局部变量表中索引位置为`1`和`2`的数据重新加载到操作数栈中并进行`iadd`加操作，将得到的结果值存到操作数栈中，PC寄存器中的值也由`6`->`7`->`8`->`9`；
